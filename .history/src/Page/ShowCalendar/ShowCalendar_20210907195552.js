...
export default function ShowCalendar() {
  ...
  React.useEffect(() => {
    CommonAction.loadingChange(true);
    TaskActions.fectListTaskRequest(dispatch);
    CommonAction.loadingChange(false);
  }, []);
        ...
        {data.map((value, key) => {
          ...
          return (
            <Button
              key={key}
              onClick={() => {
                dispatch(TaskActions.changeTaskOneDate(taskClone));
                ChangeChooseDate(value, month, year);
              }}
              style={stylex}
              variant={tyle}>
              <Link style={{ width: '100%', textAlign: 'center' }} to='/tasks'>
                {value}
              </Link>
            </Button>
          );
  ...
  const control = (month, year) => {
    return (
      <Grid style={{ textAlign: 'center' }} container>
        <Grid item xs={3}>
          <Button>
            <SkipPreviousRoundedIcon onClick={() => controlState('year', -1)} />
          </Button>
          <Button>
            <ArrowLeftIcon onClick={() => controlState('month', -1)} />
          </Button>
        </Grid>
        ...
        <Grid item xs={3}>
          <Button>
            <ArrowRightIcon onClick={() => controlState('month', 1)} />
          </Button>
          <Button>
            <SkipNextRoundedIcon onClick={() => controlState('year', 1)} />
          </Button>
        </Grid>
      </Grid>
    );
